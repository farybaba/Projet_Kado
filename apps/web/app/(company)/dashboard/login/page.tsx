...
export default function DashboardLoginPage() {
  ...
  const verifyOtp = useCallback(async (code: string) => {
    ...
    try {
      ...
      if (payload.role !== 'COMPANY_ADMIN' && payload.role !== 'COMPANY_VIEWER') {
        setOtpError('Ce numéro n\'est pas un compte RH entreprise.');
        setOtp(['', '', '', '', '', '']);
        return;
      }

      localStorage.setItem('company_token', data.accessToken);
      localStorage.setItem('company_refresh_token', data.refreshToken ?? '');
...
const s: Record<string, React.CSSProperties> = {
  ...
  back: {
    ...
    fontSize: 14, cursor: 'pointer', padding: '0 0 18px', alignSelf: 'flex-start',
  },
